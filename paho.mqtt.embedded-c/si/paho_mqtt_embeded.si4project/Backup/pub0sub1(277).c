...
typedef struct{
    ...
    char *dev_reg_js;
    char client_id[37];
    unsigned char buf[CLIENT_BUF_LEN];
    int port;
    int req_qos;
    int mysock;
    int msgid;
...
void send_hearbeat_packet(int signo)
{
    ...
    if(++time_ht_cnt >= 2){
		...
		memset(client_data.buf,0x0,CLIENT_BUF_LEN);
        publish_client("/test", dev_reg_js);
...
{/*1--sucessful, 0--faild */
    ...
    client_msg->mysock = 0;
    client_msg->data.password.cstring = generate_password_key("4CDB714CS0020","DX-1002H2N-QS","V20200330R4970_f1106_20");
...
{/*1--sucessful, 0--faild */
    ...
    do{
        	...
        	/* wait for connack */
            /*while(loop_cnt--)*/{
            	if (rc = MQTTPacket_read(client_msg->buf, CLIENT_BUF_LEN, transport_getdata) == CONNACK)
            	{
...
static void *subscribe_client_thread(void *threadid)
{
    ...
    len = MQTTSerialize_disconnect(client_data.buf, CLIENT_BUF_LEN);
	rc = transport_sendPacketBuffer(client_data.mysock, client_data.buf, len);
...
static int publish_client(char *topic,char* payload)
{
    ...
    if(payload){
		    len = MQTTSerialize_publish(client_data.buf, CLIENT_BUF_LEN, 0, 0, 0, 0, client_data.topicString, (unsigned char*)payload, strlen(payload));
		    rc = transport_sendPacketBuffer(client_data.mysock, client_data.buf, len);
		}
