...
static MQTT_CLIENT_STRU client_data = {NULL,NULL,{0},{0},{0},0,0,0,0,MQTTPacket_connectData_initializer,MQTTString_initializer};
...
static void send_hearbeat_packet(int signo)
{
    ...
    if(++time_ht_cnt >= 2){
        ...
        dev_reg_data.cpu_model = "hi3536c";
        dev_reg_data.hardware_version = "hd_1.0";
        dev_reg_data.software_version = "7927D19403902";
        dev_reg_data.mfrs = "yunding";
        dev_reg_data.sn = "7927D19403902";
        dev_reg_data.model = "DX-1002H2N-QS";
        dev_reg_js = device_register(&dev_reg_data);

		memset(client_data.tx_buf,0x0,MQTT_CLIENT_BUF_LEN);
...
static int client_init(MQTT_CLIENT_STRU *client_msg)
{/*1--sucessful, 0--faild */
    ...
    client_msg->mysock = -1;
    client_msg->data.password.cstring = generate_password_key("7927D19403902","YUNDING360","V20190522_R4073_n0005_HMR4106T20190522_20");
    ...
    client_msg->data.clientID.cstring = "yunding360_7927D19403902"; //client_msg->client_id;
    ...
    client_msg->data.username.cstring = "7927D19403902";//"4CDB714CS0020";
...
static int client_conc_to_server(MQTT_CLIENT_STRU *client_msg)
{/*1--sucessful, 0--faild */
    ...
    do{
            ...
            client_msg->data.will.message.cstring = "I am LWT msg: offline";   //ttt
            client_msg->data.will.topicName.cstring = "/will";
            client_msg->data.willFlag = 1;
            len = MQTTSerialize_connect(client_msg->tx_buf, MQTT_CLIENT_BUF_LEN, &(client_msg->data));
        	...
        	/* wait for connack */
            /*while(loop_cnt--)*/{
            	if (rc = MQTTPacket_read(client_msg->rx_buf, MQTT_CLIENT_BUF_LEN, transport_getdata) == CONNACK)
            	{
            		unsigned char sessionPresent, connack_rc;

            		if (MQTTDeserialize_connack(&sessionPresent, &connack_rc, client_msg->rx_buf, MQTT_CLIENT_BUF_LEN) != 1 || connack_rc != 0)
            		{
...
static void *subscribe_client_thread(void *threadid)
{
	...
	len = MQTTSerialize_subscribe(client_data.tx_buf, MQTT_CLIENT_BUF_LEN, 0, \
    ...
    while(loop_cnt){
    	if (MQTTPacket_read(client_data.rx_buf, MQTT_CLIENT_BUF_LEN, transport_getdata) == SUBACK){
    		unsigned short submsgid;
    		int subcount;
    		int granted_qos;

    		rc = MQTTDeserialize_suback(&submsgid, 1, &subcount, &granted_qos, client_data.rx_buf, MQTT_CLIENT_BUF_LEN);
    		//if (granted_qos != 0)
    		{
    ...
    prctl(PR_SET_NAME,("sub_2thrd"));
	while (!toStop)
	{
		memset(client_data.rx_buf,0x0,MQTT_CLIENT_BUF_LEN);
		if (MQTTPacket_read(client_data.rx_buf, MQTT_CLIENT_BUF_LEN, transport_getdata) == PUBLISH)
		{
			rc = MQTTDeserialize_publish(&dup, &qos, &retained, &msgid, &receivedTopic,\
					&payload_in, &payloadlen_in, client_data.rx_buf, MQTT_CLIENT_BUF_LEN);
    ...
    len = MQTTSerialize_disconnect(client_data.tx_buf, MQTT_CLIENT_BUF_LEN);
	rc = transport_sendPacketBuffer(client_data.mysock, client_data.tx_buf, len);
...
static int publish_client(char *topic,char* payload)
{
    ...
    if(payload){
            client_data.msgid = 218;
		    len = MQTTSerialize_publish(client_data.tx_buf, MQTT_CLIENT_BUF_LEN, 0, client_data.req_qos, 0, client_data.msgid , \
                client_data.topicString, (unsigned char*)payload, strlen(payload));
            EDWIN_INFO("publish len:%u\n",len);
		    rc = transport_sendPacketBuffer(client_data.mysock, client_data.tx_buf, len);
...
int main(int argc, char *argv[])
{
    ...
    if (argc > 1)
		client_data.host = argv[1];

	if (argc > 2){
		tmp = atoi(argv[2]);
        client_data.req_qos = tmp < 3 ? tmp : 0;
	}
